...
import static type.sglEErrorCode.SGL_OUT_OF_MEMORY;
...
public class Main
{
	...
	String sglGetErrorString(sglEErrorCode error)
	{
...
//	        SGL_PROJECTION - desc
	...
	void sglLoadMatrix(float[] matrix)
	...
	// z	- center.z
	// r	- radius
	void sglSphere(float x, float y, float z, float r)
	{

	}

	//Material ******
	void sglMaterial(float r, float g, float b, float kd, float ks, float shine, float T, float ior)
	{
	...
	void sglPointLight(float x, float y, float z, float r, float g, float b)
	{
	...
	void sglEnvironmentMap(int width, int height, float[] texels)
	{

	}

	// ?
	void sglEmissiveMaterial(float r, float g, float b, float c0, float c1, float c2)
	{
	...
	boolean in_range(int number, int low, int high)
	...
	Context current()
	{ return manager.contexts.get(manager.current); }
